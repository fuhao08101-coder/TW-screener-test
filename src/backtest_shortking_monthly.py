"""
短線王正式定案版本(大盤濾網+雙買+乖離5%+每日前3名)的逐月損益拆解回測。

目的:整段期間的「平均期望值」是正的,不代表每個月都賺錢,可能是某幾個月
大賺、某幾個月小賠,加總起來才是正的。這支回測把每一筆交易依照「出場月份」
分組,統計出每個月的損益加總,才能看出有沒有連續虧損的月份、最慘的月份
虧多少,這是「平均期望值」這個數字完全看不出來的資訊。

沿用已定案的規則:
  ATR14>=8、收盤>15MA、乖離>=5%、收盤>近5日高點、大盤環境濾網、
  外資融資雙買(9日內)、每日只取乖離最大的前3名(已驗證3檔優於6/10檔)

用法: python src/backtest_shortking_monthly.py --start 2025-08-01 --end 2026-08-31 --max-stocks 100
"""
...
def print_monthly_breakdown(trades: list[dict], start_date: date, end_date: date):
    """依「出場月份」把交易分組,統計每個月的損益加總、筆數、勝率"""
    print("\n" + "=" * 70)
    print(f"逐月損益拆解({start_date}~{end_date})")
    print("=" * 70)

    monthly = defaultdict(list)
    for t in trades:
        exit_month = t["exit_date"][:7]  # "YYYY-MM"
    ...
    print(f"\n{'月份':<10}{'筆數':>6}{'勝率':>8}{'當月損益合計':>14}{'累積損益':>12}")
    print("-" * 55)
    ...
    print("\n" + "=" * 70)
    if negative_months:
        print(f"⚠️ 共有 {len(negative_months)} 個月份是負的(當月損益加總為負):")
        for month, month_sum, total in sorted(negative_months, key=lambda x: x[1]):
            print(f"  {month}: {month_sum:+.2f}%({total}筆交易)")
        worst_month = min(negative_months, key=lambda x: x[1])
        print(f"\n最慘的月份: {worst_month[0]},當月損益合計 {worst_month[1]:+.2f}%")
    else:
        print("✅ 這段期間,沒有任何一個月份是負的,每個月加總都是正貢獻。")
    ...
    for month in sorted_months:
        if sum(t["return_pct"] for t in monthly[month]) < 0:
            ...
            current_streak = 0
    print(f"\n最長連續虧損月數: {max_consecutive_loss} 個月")
...
if __name__ == "__main__":
    ...
    print_monthly_breakdown(trades, start_d, end_d)
