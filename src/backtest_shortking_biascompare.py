...
乖離門檻三方比較回測:
  A. 乖離 >= 8%(現行版本)
  B. 乖離 >= 5%
  C. 無乖離限制(不管乖離正負,只要其他條件符合就算候選)

其他條件完全不變:ATR14>=8、收盤>15MA、收盤>近5日高點、大盤環境濾網、
外資融資雙買(9日內)、每日只取乖離最大的前6名(C組即使乖離是負的,
一樣排序取前6名,不會因為乖離很小或負的就特別排除)。

用法: python src/backtest_shortking_biascompare.py --start 2025-08-01 --end 2026-08-31 --max-stocks 100
"""
...
def run_coordinated_simulation(all_stock_data: dict, master_dates: list[str], dualbuy_by_day: dict,
                                bias_threshold: float | None, variant_name: str) -> list[dict]:
    trades = []
    positions = {}
    ...
    for day_idx, date_key in enumerate(master_dates):
        ...
        for ticker, data in all_stock_data.items():
            ...
            if bias_val is None:
                continue
            if bias_threshold is not None and bias_val < bias_threshold:
                continue  # C組(bias_threshold=None)不會進到這裡,永遠通過
...
def run_backtest(start_date: date, end_date: date, max_stocks: int | None = None):
    ...
    print(f"\n共 {len(all_stock_data)} 檔股票準備完成,開始逐日模擬(3種乖離門檻)...")
    ...
    all_trades = []
    for variant_name, threshold in BIAS_VARIANTS.items():
        print(f"\n模擬 {variant_name}(門檻={threshold})...")
        trades = run_coordinated_simulation(all_stock_data, master_dates, dualbuy_by_day,
                                             threshold, variant_name)
...
def print_report(trades: list[dict], start_date: date, end_date: date):
    print("\n" + "=" * 70)
    print(f"乖離門檻三方比較({start_date}~{end_date})")
    print("=" * 70)
    for variant_name in BIAS_VARIANTS:
