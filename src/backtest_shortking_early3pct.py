"""
出場停利觸發時機 A/B 對照回測,加上整體資金曲線最大回撤(MDD)計算。

  A組(現行規則):持有滿6天,才第一次檢查獲利有沒有到3%,達標才進入
    「創新高、防守線移到前一根K棒低點」的移動防守階段。
  B組(新規則):不設6天門檻,只要獲利隨時超過3%,立刻進入移動防守階段,
    不用等到第6天(如果到第6天都還沒達標,一樣視為時間停損出場)。

其餘規則沿用已定案版本:ATR14>=8、收盤>15MA、乖離>=5%、收盤>近5日高點、
大盤環境濾網、外資融資雙買(9日內)、每日只取乖離最大的前3名。

MDD計算方式:把所有交易依出場日期排序,模擬資金從1.0開始,每筆交易依報酬率
複利成長,計算資金曲線從歷史高點回落的最大百分比。這是簡化模型(沒有模擬
「3檔同時持有時的資金分配」),用來比較A/B兩組的相對回撤差異,不是精確的
真實資金回撤金額。

用法: python src/backtest_shortking_early3pct.py --start 2025-08-01 --end 2026-08-31 --max-stocks 100
"""
...
def run_coordinated_simulation(all_stock_data: dict, master_dates: list[str], dualbuy_by_day: dict,
    ...
    """
    variant:
      "A_6天檢查"(現行規則):持有滿6天,才第一次檢查3%達標與否,達標才進入
        移動防守階段(創新高、防守線移到前一根K棒低點)
      "B_隨時3%啟動"(新規則):不設6天門檻,只要獲利隨時超過3%,立刻進入
        移動防守階段,不用等到第6天。如果持有滿6天依然沒有到3%,一樣視為
        時間停損出場(這條規則兩組共用,不變)
    """
    trades = []
    positions = {}
    ...
    for day_idx, date_key in enumerate(master_dates):
        ...
        for ticker, pos in positions.items():
            ...
            if not pos["activated"]:
                ...
                if variant == "B_隨時3%啟動" and ret_now >= PROFIT_THRESHOLD_PCT:
                    # B組:不用等滿6天,隨時只要達標就立刻啟動移動防守
                    pos["activated"] = True
                    pos["highest_close"] = c
...
def run_backtest(start_date: date, end_date: date, max_stocks: int | None = None):
    ...
    print("\n模擬 A_6天檢查(現行規則)...")
    trades_a = run_coordinated_simulation(all_stock_data, master_dates, dualbuy_by_day, "A_6天檢查")
    print(f"產生 {len(trades_a)} 筆交易")

    print("\n模擬 B_隨時3%啟動(新規則)...")
    trades_b = run_coordinated_simulation(all_stock_data, master_dates, dualbuy_by_day, "B_隨時3%啟動")
    print(f"產生 {len(trades_b)} 筆交易")
...
def print_full_report(all_trades: list[dict], start_date: date, end_date: date):
    print("\n" + "=" * 70)
    print(f"A組(6天檢查) vs B組(隨時3%啟動)完整比較({start_date}~{end_date})")
    print("=" * 70)

    for variant in ["A_6天檢查", "B_隨時3%啟動"]:
