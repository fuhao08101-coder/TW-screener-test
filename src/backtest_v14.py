"""
回測工具 v14:V10-C核心價格條件,但突破天數從5日改成3日
(ATR14≥8、收盤>15MA、盤中突破近3日高點)
+ 外資融資雙買濾網(上市TWSE+上櫃TPEX都涵蓋,近9個交易日內雙買、之後未雙減),
出場沿用v10-C的規則。

回測範圍最近2年,上市+上櫃都回測——外資融資雙買濾網的
TPEX(上櫃)資料源已經驗證過欄位格式正確(CSV格式,代號/前資餘額/資餘額)。
上次(v12,突破5日)加上上櫃會讓期望值從+1.36%降到+1.06%,這次條件本身
改了(3日突破),重新驗證一次上櫃納入後的影響是否仍然一樣。
...
STALL_DAYS_LIMIT = 2
DUAL_BUY_WINDOW = 9
HISTORY_PERIOD = "3y"  # 涵蓋2年回測期+均線計算的緩衝


def simulate_trades_v14(df: pd.DataFrame, ticker: str, qualified_dates: set[str]) -> list[dict]:
...
def run_backtest_v14(max_stocks: int | None = None):
    ...
    print(f"共 {len(universe)} 檔,開始回測(v14:3日突破+外資融資雙買,2年)...")
    ...
    for batch_idx, batch in enumerate(batches, 1):
        ...
        for t in batch:
            ...
            try:
                trades = simulate_trades_v14(df, t, qualified_dates)
...
def print_stats_v14(trades: list[dict]):
    print("\n" + "=" * 60)
    print("回測結果統計 v14(3日突破+外資融資雙買,近2年,上市+上櫃)")
    print("=" * 60)

    print("\n【對照:v10-C(無雙買濾網,5年,含上市+上櫃)】")
    print("  3900筆, 勝率37.1%, 期望值+1.30%")

    _stats_for(trades, "v14 全部交易")

    twse_trades = [t for t in trades if t.get("market") == "TWSE"]
    tpex_trades = [t for t in trades if t.get("market") == "TPEX"]
    _stats_for(twse_trades, "v14 只看上市(TWSE)")
    _stats_for(tpex_trades, "v14 只看上櫃(TPEX)")
...
if __name__ == "__main__":
    ...
    trades = run_backtest_v14(max_stocks=args.max_stocks)
    print_stats_v14(trades)
